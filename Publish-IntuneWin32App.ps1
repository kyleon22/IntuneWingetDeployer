...
function Connect-IntuneGraph { if ($UseIntuneGraphAuth) { Write-Section 'Connecting with IntuneWin32App auth'; if (-not $IntuneClientId) { Stop-WithError 'IntuneClientId is required when -UseIntuneGraphAuth is specified.' }; Remove-Module IntuneWin32App -ErrorAction SilentlyContinue; Import-PreferredIntuneModule; $authParams=@{ TenantID=$TenantId; ClientID=$IntuneClientId }; if ($IntuneDeviceCode) { $authParams.DeviceCode = $true } else { $authParams.Interactive = $true }; Connect-MSIntuneGraph @authParams | Out-Null; return }; Write-Section 'Connecting to Microsoft Graph'; Connect-MgGraph -TenantId $TenantId -Scopes @('DeviceManagementApps.ReadWrite.All','DeviceManagementConfiguration.ReadWrite.All','Group.Read.All') | Out-Null; Remove-Module IntuneWin32App -ErrorAction SilentlyContinue; Import-PreferredIntuneModule }
