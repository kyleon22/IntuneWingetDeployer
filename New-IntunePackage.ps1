...
function Save-Installer { param([string]$Url,[string]$Destination,[string]$SourcePath) if ($SourcePath) { if (-not (Test-Path -LiteralPath $SourcePath)) { Stop-WithError "Local installer path not found: $SourcePath" }; Write-Section 'Using local installer'; Copy-Item -LiteralPath $SourcePath -Destination $Destination -Force; Write-Log -Message "Installer copied from local path to $Destination" -Level SUCCESS; return }; if (-not $Url) { Stop-WithError 'No installer URL or local installer path was provided.' }; Write-Section 'Downloading installer'; Invoke-WebRequest -Uri $Url -OutFile $Destination; Write-Log -Message "Installer downloaded to $Destination" -Level SUCCESS }
